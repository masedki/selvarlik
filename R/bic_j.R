bic_j <- function(j,x,S, both=TRUE)
{
  ...
  else
  {
    ...
    biggest <- formula(lm(x[,j]~. ,data=as.data.frame(x[,S])))
    if(both)
      {reg.model = step(min.model, direction='both', k = log(nrow(x)), scope=biggest, trace=FALSE)} else{
